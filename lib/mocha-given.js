...
// Bundlers resolve a bare `require('mocha')` eagerly regardless of the guards
// around it, which broke the `browser` entry point. Keep the lookup dynamic.
const resolveMocha = () => {
  if (typeof module !== 'undefined' && module.parent) {
    return module.parent.require('mocha');
  }
  if (typeof window !== 'undefined' && window.Mocha) {
    return window.Mocha;
  }
  // eslint-disable-next-line no-eval -- opaque to bundlers on purpose
  const req = typeof require === 'function' ? require : eval('require');
  return req('mocha');
};

const Mocha = resolveMocha();

const Suite = Mocha.Suite;
const Test = Mocha.Test;
const Context = Mocha.Context;

// Mocha.utils.escapeRegexp was removed; it is absent across the whole
// supported peer range, so calling it aborted the run at spec load time.
...
class Waterfall {
  ...
  // done(err) has to fail the test. Ignoring the argument made an errored
  // When or Invariant indistinguishable from a successful one.
  asyncTaskCompleted = (err) => {
  ...
  invokeFinalCallbackIfNecessary = () => {
    if (this.asyncCount === 0 && !this.settled) {
      this.settled = true;
      const finish = this.finalCallback;
      this.finalCallback = undefined;
      finish?.apply(this.context);
    }
  };
  ...
  flow = () => {
    ...
    if (func.length > 0) {
      if (isAsyncFunction(func)) return this.fail(new Error(BOTH_STYLES));

      let returned;
      try {
        returned = func.apply(this.context, [this.asyncTaskCompleted]);
      } catch (err) {
        return this.fail(err);
      }
      if (isThenable(returned) && !this.settled) {
        this.fail(new Error(BOTH_STYLES));
      }
      return;
    }
    ...
    let returned;
    try {
      returned = func.apply(this.context);
    } catch (err) {
      return this.fail(err);
    }
    ...
    if (isThenable(returned)) {
      ...
      returned.then(() => {
        this.asyncCount -= 1;
        // a synchronous throw from a later step would otherwise become an
        // unhandled rejection and surface as a bogus timeout
        try {
          this.flow();
        } catch (err) {
          this.fail(err);
...
// An async function that also declares `done` is ambiguous, and it has to be
// caught before the body runs: once done() has fired, the step has already
// completed and reporting the error would call mocha's done twice.
...
const runAssertions = (ctx, assertions, args, done) => {
  ...
  const next = () => {
    ...
    if (fn.length > 0) {
      if (isAsyncFunction(fn)) return done(new Error(BOTH_STYLES));
      ...
      if (isThenable(returned) && !settled) {
        return done(new Error(BOTH_STYLES));
      }
      return;
    }
...
const addAssertion = (group, specArgs) => {
  ...
  if (time !== undefined) {
    group.time = time;
    group.timelabel =
      time > 1e3 ? `after ${time / 1e3} s, ` : `after ${time} ms, `;
  }
...
const declareSpec = (specArgs, itFunc) => {
  ...
  group.test = itFunc(titleFor(group), function (done) {
    ...
    // Invariants are assertions, so a falsy return has to fail. When steps are
    // setup and legitimately return anything, so they stay in the waterfall.
    const invariants = invariantList.map((fn) => ({ fn, label: undefined }));
    const assertInvariants = () =>
      runAssertions(this, invariants, args, (err) => {
    ...
    new Waterfall(this, whenList.slice(0), assertInvariants, done).flow();
  });
...
const MochaGivenSuite = (suite) => {
  ...
  suite.on('pre-require', (context, file, mocha) => {
    ...
    context.xdescribe =
      ...
      context.describe.skip =
        (title, fn) => {
          ...
          nested.pending = true;
          suites.unshift(nested);
          // save/restore as context.describe does, otherwise an And written
          // after a skipped block attaches to a Then inside it and is never run
          const outerThen = currentThen;
          currentThen = null;
          fn.call(nested);
          currentThen = outerThen;
    ...
    // mocha's own mechanism. Grepping on the title cannot work: a suite title
    // is a prefix of its tests' full titles, so no anchoring distinguishes
    // "payments" from "payments extended".
    context.describe.only = (title, fn) => {
      const nested = context.describe(title, fn);
      nested.markOnly ? nested.markOnly() : mocha.grep(nested.fullTitle());
      return nested;
    };
    ...
    context.it.only = (title, fn) => {
      ...
      test.markOnly
        ? test.markOnly()
        : mocha.grep(new RegExp('^' + escapeRegexp(test.fullTitle()) + '$'));
      return test;
    };
