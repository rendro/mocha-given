...
var Context, Mocha, MochaGivenSuite, Suite, Test, comparisonLookup, declareSpec, finalStatementFrom, getErrorDetails, invariantList, o, stringifyExpectation, utils, wasComparison, whenList,
...
declareSpec = function(specArgs, itFunc) {
  ...
  return itFunc("then " + (label != null ? label : stringifyExpectation(fn)), function(done) {
    var i, _i, _j, _len, _len1;
    if (whenList.length) {
      for (_i = 0, _len = whenList.length; _i < _len; _i++) {
        i = whenList[_i];
        i.apply(this);
      }
    }
    if (invariantList.length) {
      for (_j = 0, _len1 = invariantList.length; _j < _len1; _j++) {
        i = invariantList[_j];
        i.apply(this);
      }
    }
    o(fn).assert(this, Array.prototype.slice.call(arguments));
...
MochaGivenSuite = function(suite) {
  ...
  return suite.on('pre-require', function(context, file, mocha) {
    ...
    context.Then.only = function() {
      return declareSpec(arguments, this.it.only);
    };
